...
public final class ParallelShortestPathFinder {
    ...
    public ParallelShortestPathFinder(Map<WorldPoint, List<Transport>> transportCoords, CollisionMap collisionMap) {
        this(transportCoords, collisionMap, a -> ForkJoinPool.commonPool().submit(a));
    }

    public ParallelShortestPathFinder(Map<WorldPoint, List<Transport>> transportCoords, CollisionMap collisionMap, Function<Runnable, Future<?>> executor) {
    ...
    public List<WorldPoint> search(List<WorldPoint> start, WorldPoint end) {
        ...
        return start.stream().map(s -> pool.submit(() -> search(s, end, finished, visited, bestCompletePath))).map(fut -> {
            List<WorldPoint> out;
            try {
                out = fut.get();
            } catch (Exception e) {
    ...
    private List<WorldPoint> search(WorldPoint start, WorldPoint end, AtomicBoolean finished, Set<WorldPoint> visited, AtomicStampedReference<WorldPoint> bestCompletePath) {
        ...
        Future<?> future = executor.apply(() -> {
            new Search(start, end, finished, visited, bestCompletePath, paths1, paths2, this::getNeighbours, this::heuristic).run();
        ...
        new Search(end, start, finished, visited, bestCompletePath, paths2, paths1, this::getNeighbours, this::heuristic).run();
        var commonNode = bestCompletePath.getReference();
        if (commonNode == null) {
            future.cancel(true);
        ...
        return ImmutableList.<WorldPoint>builderWithExpectedSize(paths1.size() + paths2.size()).addAll(result1).addAll(result2).build();
    }
