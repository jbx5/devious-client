		...
		if (hadNetworkError) {
			...
			if (gameState == 30) {
				...
				while (class216.method4239()) {
					var14 = SecureUrlRequester.getPacketBufferNode(ClientPacket.REFLECTION_CHECK_REPLY, packetWriter.isaacCipher);
				...
				if (timer.field4596) {
					var14 = SecureUrlRequester.getPacketBufferNode(ClientPacket.LOGIN_TIMINGS, packetWriter.isaacCipher);
					var14.packetBuffer.writeByte(0);
					var2 = var14.packetBuffer.offset;
				...
				if (MouseHandler.MouseHandler_lastButton == 1 || !HttpRequestTask.mouseCam && MouseHandler.MouseHandler_lastButton == 4 || MouseHandler.MouseHandler_lastButton == 2) {
					...
					var5 = (int)var16;
					PacketBufferNode var18 = SecureUrlRequester.getPacketBufferNode(ClientPacket.EVENT_MOUSE_CLICK, packetWriter.isaacCipher);
				...
				if (field724.field2354 > 0) {
					var14 = SecureUrlRequester.getPacketBufferNode(ClientPacket.EVENT_KEYBOARD, packetWriter.isaacCipher);
				...
				if (field587 && field523 <= 0) {
					field523 = 20;
					field587 = false;
					var14 = SecureUrlRequester.getPacketBufferNode(ClientPacket.EVENT_CAMERA_POSITION, packetWriter.isaacCipher);
				...
				if (class357.hasFocus && !hadFocus) {
					hadFocus = true;
					var14 = SecureUrlRequester.getPacketBufferNode(ClientPacket.EVENT_APPLET_FOCUS, packetWriter.isaacCipher);
															...
															while (true) {
																...
																if (var48 == null) {
																	...
																	if (Scene.shouldSendWalk()) {
																		...
																		var26 = SecureUrlRequester.getPacketBufferNode(ClientPacket.MOVE_GAMECLICK, packetWriter.isaacCipher);
																	...
																	if (var5 > 15000 && var6 > 15000) {
																		...
																		var26 = SecureUrlRequester.getPacketBufferNode(ClientPacket.EVENT_MOUSE_IDLE, packetWriter.isaacCipher);
																		packetWriter.addNode(var26);
																	}
																	...
																	if (packetWriter.pendingWrites > 50) {
																		var26 = SecureUrlRequester.getPacketBufferNode(ClientPacket.NO_TIMEOUT, packetWriter.isaacCipher);
																		packetWriter.addNode(var26);
																	}
		...
		if (var2 == null) {
			...
			try {
				...
				if (ServerPacket.PING_STATISTICS_REQUEST == var1.serverPacket) {
					...
					PacketBufferNode var79 = SecureUrlRequester.getPacketBufferNode(ClientPacket.PING_STATISTICS, packetWriter.isaacCipher);
			...
			if (MouseHandler.MouseHandler_currentButton == 0) {
				if (isDraggingWidget) {
					...
					if (draggedOnWidget != null && class227.method4434(clickedWidget) != null) {
						PacketBufferNode var9 = SecureUrlRequester.getPacketBufferNode(ClientPacket.OPHELDD, packetWriter.isaacCipher);
