import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("mm")
@Implements("ClientPacket")
public class ClientPacket implements class321 {
	@ObfuscatedName("ap")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3383;
	@ObfuscatedName("aw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3379;
	@ObfuscatedName("ak")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_MOUSE_IDLE;
	@ObfuscatedName("aj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER7;
	@ObfuscatedName("ai")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3315;
	@ObfuscatedName("ay")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON2;
	@ObfuscatedName("as")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3397;
	@ObfuscatedName("ae")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3318;
	@ObfuscatedName("am")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYERT;
	@ObfuscatedName("at")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ2;
	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC1;
	@ObfuscatedName("an")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3322;
	@ObfuscatedName("ao")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket REFLECTION_CHECK_REPLY;
	@ObfuscatedName("af")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON1;
	@ObfuscatedName("ar")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJE;
	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER1;
	@ObfuscatedName("az")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC2;
	@ObfuscatedName("ag")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_COUNTDIALOG;
	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON3;
	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FRIEND_DELUSER;
	@ObfuscatedName("av")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket NO_TIMEOUT;
	@ObfuscatedName("ax")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3332;
	@ObfuscatedName("aq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3333;
	@ObfuscatedName("al")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON8;
	@ObfuscatedName("aa")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_WINDOW_SETTING;
	@ObfuscatedName("ah")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MOVE_GAMECLICK;
	@ObfuscatedName("bh")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ1;
	@ObfuscatedName("bj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3338;
	@ObfuscatedName("bv")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CHAT_SENDABUSEREPORT;
	@ObfuscatedName("bx")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLOSE_MODAL;
	@ObfuscatedName("bk")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ5;
	@ObfuscatedName("bb")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPCT;
	@ObfuscatedName("bq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket DOCHEAT;
	@ObfuscatedName("bp")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_SUBOP;
	@ObfuscatedName("bz")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC3;
	@ObfuscatedName("bc")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3317;
	@ObfuscatedName("by")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_KEYBOARD;
	@ObfuscatedName("br")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_APPLET_FOCUS;
	@ObfuscatedName("bn")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3349;
	@ObfuscatedName("bu")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3350;
	@ObfuscatedName("bt")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC3;
	@ObfuscatedName("bs")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON5;
	@ObfuscatedName("bm")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3385;
	@ObfuscatedName("bo")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER6;
	@ObfuscatedName("bd")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER8;
	@ObfuscatedName("ba")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPCE;
	@ObfuscatedName("bw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON7;
	@ObfuscatedName("bi")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC5;
	@ObfuscatedName("bf")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3359;
	@ObfuscatedName("bg")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER3;
	@ObfuscatedName("bl")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket DETECT_MODIFIED_CLIENT;
	@ObfuscatedName("be")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC4;
	@ObfuscatedName("cq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOCT;
	@ObfuscatedName("cb")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER5;
	@ObfuscatedName("cd")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_PAUSEBUTTON;
	@ObfuscatedName("ci")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_MOUSE_CLICK;
	@ObfuscatedName("cn")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON6;
	@ObfuscatedName("co")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3368;
	@ObfuscatedName("cf")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC1;
	@ObfuscatedName("cl")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket LOGIN_TIMINGS;
	@ObfuscatedName("cp")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CHAT_SENDPRIVATE;
	@ObfuscatedName("cv")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPHELDD;
	@ObfuscatedName("cy")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3409;
	@ObfuscatedName("cu")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_NAMEDIALOG;
	@ObfuscatedName("ce")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC5;
	@ObfuscatedName("cg")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTONT;
	@ObfuscatedName("cx")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CHAT_SETFILTER;
	@ObfuscatedName("cz")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket PING_STATISTICS;
	@ObfuscatedName("ch")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3394;
	@ObfuscatedName("ct")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FREECAM_EXIT;
	@ObfuscatedName("cc")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3381;
	@ObfuscatedName("cj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3382;
	@ObfuscatedName("cm")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MINIMAP_CLICK;
	@ObfuscatedName("cw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket WIDGET_TYPE;
	@ObfuscatedName("ca")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_STRINGDIALOG;
	@ObfuscatedName("cs")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON4;
	@ObfuscatedName("cr")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3387;
	@ObfuscatedName("ck")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3388;
	@ObfuscatedName("dq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOCE;
	@ObfuscatedName("dd")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON9;
	@ObfuscatedName("dc")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ4;
	@ObfuscatedName("da")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3392;
	@ObfuscatedName("dr")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_OBJDIALOG;
	@ObfuscatedName("dh")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FRIEND_ADDUSER;
	@ObfuscatedName("dg")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER2;
	@ObfuscatedName("df")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3408;
	@ObfuscatedName("du")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IGNORE_DELUSER;
	@ObfuscatedName("dk")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3398;
	@ObfuscatedName("dl")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJT;
	@ObfuscatedName("dm")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3400;
	@ObfuscatedName("dt")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLAN_KICKUSER;
	@ObfuscatedName("dx")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3402;
	@ObfuscatedName("ds")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER4;
	@ObfuscatedName("db")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON10;
	@ObfuscatedName("dv")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_CAMERA_POSITION;
	@ObfuscatedName("dj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC2;
	@ObfuscatedName("dp")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3407;
	@ObfuscatedName("dy")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3330;
	@ObfuscatedName("do")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC4;
	@ObfuscatedName("dw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3410;
	@ObfuscatedName("de")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ3;
	@ObfuscatedName("dz")
	@ObfuscatedGetter(
		intValue = 669283137
	)
	@Export("id")
	final int id;
	@ObfuscatedName("di")
	@ObfuscatedGetter(
		intValue = 58468603
	)
	@Export("length")
	final int length;

	static {
		field3383 = new ClientPacket(0, 26);
		field3379 = new ClientPacket(1, 13);
		EVENT_MOUSE_IDLE = new ClientPacket(2, 0);
		OPPLAYER7 = new ClientPacket(3, 3);
		field3315 = new ClientPacket(4, -1);
		IF_BUTTON2 = new ClientPacket(5, 8);
		field3397 = new ClientPacket(6, -1);
		field3318 = new ClientPacket(7, 11);
		OPPLAYERT = new ClientPacket(8, 11);
		OPOBJ2 = new ClientPacket(9, 7);
		OPLOC1 = new ClientPacket(10, 7);
		field3322 = new ClientPacket(11, 2);
		REFLECTION_CHECK_REPLY = new ClientPacket(12, -1);
		IF_BUTTON1 = new ClientPacket(13, 8);
		OPOBJE = new ClientPacket(14, 6);
		OPPLAYER1 = new ClientPacket(15, 3);
		OPNPC2 = new ClientPacket(16, 3);
		RESUME_COUNTDIALOG = new ClientPacket(17, 4);
		IF_BUTTON3 = new ClientPacket(18, 8);
		FRIEND_DELUSER = new ClientPacket(19, -1);
		NO_TIMEOUT = new ClientPacket(20, 0);
		field3332 = new ClientPacket(21, 15);
		field3333 = new ClientPacket(22, 1);
		IF_BUTTON8 = new ClientPacket(23, 8);
		EVENT_WINDOW_SETTING = new ClientPacket(24, 5);
		MOVE_GAMECLICK = new ClientPacket(25, -1);
		OPOBJ1 = new ClientPacket(26, 7);
		field3338 = new ClientPacket(27, 0);
		CHAT_SENDABUSEREPORT = new ClientPacket(28, -1);
		CLOSE_MODAL = new ClientPacket(29, 0);
		OPOBJ5 = new ClientPacket(30, 7);
		OPNPCT = new ClientPacket(31, 11);
		DOCHEAT = new ClientPacket(32, -1);
		IF_SUBOP = new ClientPacket(33, 10);
		OPNPC3 = new ClientPacket(34, 3);
		field3317 = new ClientPacket(35, -1);
		EVENT_KEYBOARD = new ClientPacket(36, -2);
		EVENT_APPLET_FOCUS = new ClientPacket(37, 1);
		field3349 = new ClientPacket(38, -1);
		field3350 = new ClientPacket(39, 2);
		OPLOC3 = new ClientPacket(40, 7);
		IF_BUTTON5 = new ClientPacket(41, 8);
		field3385 = new ClientPacket(42, 4);
		OPPLAYER6 = new ClientPacket(43, 3);
		OPPLAYER8 = new ClientPacket(44, 3);
		OPNPCE = new ClientPacket(45, 2);
		IF_BUTTON7 = new ClientPacket(46, 8);
		OPLOC5 = new ClientPacket(47, 7);
		field3359 = new ClientPacket(48, 4);
		OPPLAYER3 = new ClientPacket(49, 3);
		DETECT_MODIFIED_CLIENT = new ClientPacket(50, 4);
		OPLOC4 = new ClientPacket(51, 7);
		OPLOCT = new ClientPacket(52, 15);
		OPPLAYER5 = new ClientPacket(53, 3);
		RESUME_PAUSEBUTTON = new ClientPacket(54, 6);
		EVENT_MOUSE_CLICK = new ClientPacket(55, 6);
		IF_BUTTON6 = new ClientPacket(56, 8);
		field3368 = new ClientPacket(57, -2);
		OPNPC1 = new ClientPacket(58, 3);
		LOGIN_TIMINGS = new ClientPacket(59, -1);
		CHAT_SENDPRIVATE = new ClientPacket(60, -2);
		OPHELDD = new ClientPacket(61, 16);
		field3409 = new ClientPacket(62, -1);
		RESUME_NAMEDIALOG = new ClientPacket(63, -1);
		OPNPC5 = new ClientPacket(64, 3);
		IF_BUTTONT = new ClientPacket(65, 16);
		CHAT_SETFILTER = new ClientPacket(66, 3);
		PING_STATISTICS = new ClientPacket(67, 10);
		field3394 = new ClientPacket(68, 1);
		FREECAM_EXIT = new ClientPacket(69, 0);
		field3381 = new ClientPacket(70, 2);
		field3382 = new ClientPacket(71, 7);
		MINIMAP_CLICK = new ClientPacket(72, -1);
		WIDGET_TYPE = new ClientPacket(73, 4);
		RESUME_STRINGDIALOG = new ClientPacket(74, -1);
		IF_BUTTON4 = new ClientPacket(75, 8);
		field3387 = new ClientPacket(76, -1);
		field3388 = new ClientPacket(77, -1);
		OPLOCE = new ClientPacket(78, 2);
		IF_BUTTON9 = new ClientPacket(79, 8);
		OPOBJ4 = new ClientPacket(80, 7);
		field3392 = new ClientPacket(81, -1);
		RESUME_OBJDIALOG = new ClientPacket(82, 2);
		FRIEND_ADDUSER = new ClientPacket(83, -1);
		OPPLAYER2 = new ClientPacket(84, 3);
		field3408 = new ClientPacket(85, -1);
		IGNORE_DELUSER = new ClientPacket(86, -1);
		field3398 = new ClientPacket(87, -1);
		OPOBJT = new ClientPacket(88, 15);
		field3400 = new ClientPacket(89, 15);
		CLAN_KICKUSER = new ClientPacket(90, -1);
		field3402 = new ClientPacket(91, 9);
		OPPLAYER4 = new ClientPacket(92, 3);
		IF_BUTTON10 = new ClientPacket(93, 8);
		EVENT_CAMERA_POSITION = new ClientPacket(94, 4);
		OPLOC2 = new ClientPacket(95, 7);
		field3407 = new ClientPacket(96, 22);
		field3330 = new ClientPacket(97, 1);
		OPNPC4 = new ClientPacket(98, 3);
		field3410 = new ClientPacket(99, 11);
		OPOBJ3 = new ClientPacket(100, 7);
	}

	ClientPacket(int var1, int var2) {
		this.id = var1;
		this.length = var2;
	}

	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "(ZI)V",
		garbageValue = "-125923380"
	)
	@Export("updateLoginStatusUsernameRemembered")
	static void updateLoginStatusUsernameRemembered(boolean var0) {
		byte var1 = 0;
		if (!PlayerComposition.method6959()) {
			var1 = 12;
		} else if (Projectile.client.containsAccessAndRefreshToken() || Projectile.client.otlTokenRequesterInitialized() || Projectile.client.containsSessionAndCharacterId()) {
			var1 = 10;
		}

		class6.updateLoginIndex(var1);
		if (var0) {
			Login.Login_username = "";
			Login.Login_password = "";
			class333.otpMedium = 0;
			Huffman.otp = "";
		}

		class362.method7457();
		class96.focusPasswordWhenUsernameFilled();
	}
}
